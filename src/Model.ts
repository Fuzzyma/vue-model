import { computed, reactive, shallowReactive, toRaw, type WritableComputedRef } from 'vue';
...
export type Dehydrate<T extends typeof Model> = Properties<T> & {
  __class__: T['model'];
};
...
export type RelationPropertyNames<T extends typeof Model> = {
  [P in keyof OmitPrivate<T>]: OmitPrivate<T>[P] extends Model | Model[]
    ? OmitPrivate<T>[P] extends Function
      ? never
      : P
    : never;
}[keyof OmitPrivate<T>];
...
type returnPrimitive<T extends any> = T extends String
  ? string
  : T extends Number
  ? number
  : T extends Boolean
...
function createComputed<T, K>(
  this: ctor<K>,
  getter: (context: K) => T,
  setter = (context: K, val: any) => {
    /* */
  },
) {
...
function createHasOne<T extends Model>(
  this: ctor,
  related: ctor<T> | string,
  foreignKey: KeyName = camelCase(this.model) + 'Id',
  otherKey: KeyName = this.primaryKey,
) {
  return new HasOne<T>(this, related, foreignKey, otherKey);
}

function createHasMany<T extends Model>(
  this: ctor,
  related: ctor<T> | string,
  foreignKey: KeyName = camelCase(this.model) + 'Id',
  otherKey: KeyName = this.primaryKey,
...
const filterRelation = (fields: [string, Field<any>][]) =>
  fields.filter(([name, field]) => field instanceof Relation && !isLocal(field)) as [string, Relation][];
...
function checker<T extends typeof Model>(
  conditions: Partial<Record<PropertyNames<T>, unknown>> | ((m: InstanceType<T>) => boolean),
) {
...
export class Model {
  ...
  static whereFirst<T extends typeof Model>(
    this: T,
    condition: Partial<Record<PropertyNames<T>, unknown>> | ((m: InstanceType<T>) => boolean),
  ) {
  ...
  static getOrCreate<T extends typeof Model>(this: T, values: Record<string, unknown>[]): InstanceType<T>[];
  static getOrCreate<T extends typeof Model>(
    this: T,
  ...
  static fillOrCreate<T extends typeof Model>(this: T, values: Record<string, unknown>[]): InstanceType<T>[];
  static fillOrCreate<T extends typeof Model>(
    this: T,
    values: Record<string, unknown>[] | Record<string, unknown>,
  ): InstanceType<T>[] | InstanceType<T> {
    const isArray = Array.isArray(values);
  ...
  dehydrate<T extends typeof Model>(this: InstanceType<T>, relations = this.static().cascades() as RelationPropertyNames<T>[]) {
    const obj = {
      ...toRaw(this._cache),
      __class__: this.static().model,
    } as Dehydrate<T> & RelationProperties<T>;
...
export class ComputedField<K, T extends unknown = unknown> extends Field<T> {
  ...
  createComputed(context: K) {
    this.value = computed<T>({
      get: () => this.getter(context),
